...
## 2024-05-24 - Missing Security Headers on Static Assets in Vercel
**Vulnerability:** Global security headers (like Content-Security-Policy, X-Frame-Options) were only applied to Flask API endpoints via `@app.after_request`, leaving static assets (like `public/index.html`) completely unprotected against attacks like clickjacking and MIME-sniffing.
**Learning:** In Vercel deployments, web framework middleware only protects the framework's specific routes (e.g., `/api`). Static assets are served directly by Vercel's edge network and bypass the framework entirely.
**Prevention:** Always configure global security headers in the `vercel.json` `headers` block (using `source: "/(.*)"`) to ensure all responses, including static files, are properly protected.
