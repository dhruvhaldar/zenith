...
## 2026-05-18 - [Missing Required SECRET_KEY in Serverless Deployments]
**Vulnerability:** The application was missing a required `SECRET_KEY` configuration, which left the session setup without a secure cryptographic key. Using an ephemeral fallback (like `secrets.token_hex(32)`) instead would break stateful functionality (like sessions) on cold starts in serverless environments like Vercel because each instance gets a different key.
**Learning:** In serverless environments, an application cannot rely on generated fallback keys on startup for cryptographic signing because instances are ephemeral and state is not shared. Falling back to an insecure hardcoded string is also dangerous.
**Prevention:** Always require `SECRET_KEY` via environment variables and raise a fatal error (e.g., `RuntimeError`) during application startup if it is missing, ensuring the application fails securely and configuration errors are caught immediately.
