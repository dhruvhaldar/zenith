...
## 2026-03-31 - Missing Global Security Headers on Static Assets
**Vulnerability:** The Flask application applies security headers (like Content-Security-Policy, X-Frame-Options, X-Content-Type-Options) via a middleware (`@app.after_request`). However, in a Vercel deployment, static frontend assets (like `public/index.html` or assets in `public/assets/`) are served directly by the CDN, bypassing the Flask API middleware completely. This leaves the static application vulnerable to attacks such as Clickjacking (missing X-Frame-Options) or MIME-sniffing.
**Learning:** Web framework middlewares only protect dynamic routes processed by the framework instance. In serverless/CDN setups (like Vercel), static assets are inherently unprotected by backend code.
**Prevention:** Always configure global security headers at the infrastructure level (e.g., in the `headers` block of `vercel.json`) to guarantee protection across both dynamic endpoints and static files.
