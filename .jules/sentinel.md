...
## 2026-04-17 - [HTTP Parameter Pollution (HPP) in Query Parsing]
**Vulnerability:** The API retrieved query parameters using `request.args.get('mag')`, which defaults to returning the first supplied value when multiple identically-named parameters exist (e.g., `?mag=12.0&mag=invalid`). However, if an attacker provides multiple values, they might bypass validation checks depending on how upstream WAFs or reverse proxies parse the query string, leading to HTTP Parameter Pollution (HPP) and potential TypeErrors or value injection downstream.
**Learning:** In Flask/Werkzeug, `request.args` is a `MultiDict`. When using `.get()`, it silently discards additional values, which can lead to inconsistencies between what a security scanner/proxy sees and what the application processes.
**Prevention:** Explicitly use `.getlist()` and reject the request (throw an error) if an array of multiple values is unexpectedly provided for a key meant to be a singular input.
