...
**Vulnerability:** The API implements rate limiting and sets corresponding headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, etc.), but these headers were not included in `Access-Control-Expose-Headers`. As a result, legitimate cross-origin browser clients could not read these headers. This prevents well-behaving clients from proactively throttling their requests, leading to unnecessary 429 responses and potential self-inflicted Denial of Service (DoS) states on the server.
**Learning:** For rate limiting metadata to be effective as a defense-in-depth measure against accidental DoS by web clients, those clients must be able to read the headers. In a CORS context, this requires explicitly exposing them.
**Prevention:** When exposing a rate-limited API to cross-origin clients (CORS), explicitly include the rate limit headers (e.g., `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `Retry-After`) in the `Access-Control-Expose-Headers` response header.
