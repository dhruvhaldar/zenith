...
## 2024-05-24 - [HTTP Parameter Pollution (HPP) via request.args.get()]
**Vulnerability:** The API used `request.args.get(key)` to parse query string parameters. In Flask/Werkzeug, this method silently discards all but the first value if multiple values are provided for the same key (e.g., `?mag=12&mag=15`). This can lead to HTTP Parameter Pollution (HPP) bypasses if an upstream WAF or proxy parses the query string differently (e.g., combining them, or taking the last value) than the Flask backend.
**Learning:** Using `request.args.get(key)` is insecure for sensitive parameter extraction because it masks the presence of duplicate keys, allowing HPP payloads to bypass single-value validation.
**Prevention:** In Flask/Werkzeug applications, prevent HPP vulnerabilities by using `request.args.getlist(key)` and explicitly rejecting requests with multiple values for the same key.
