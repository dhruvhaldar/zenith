...
## 2025-04-19 - HTTP Parameter Pollution (HPP) in Query Parsing
**Vulnerability:** Safe get float helper used `args.get(key)` which silently discards extra duplicate keys from requests (e.g. `?mag=10&mag=20`), returning only the first.
**Learning:** Using `request.args.get()` in Werkzeug applications allows attackers to bypass upstream WAF or validation rules if those systems interpret duplicate parameters differently than the backend application.
**Prevention:** Use `args.getlist(key)` instead and explicitly raise an error or reject the request if multiple values are detected.
