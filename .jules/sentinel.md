...
**Learning:** Returning a `400` status code is sufficient for the client, but insufficient for security monitoring. Validation logic must fail loudly in the backend logs to create an audit trail.
**Prevention:** When validating API inputs, explicitly log the failure (e.g., `app.logger.warning`) alongside the request context (`request.remote_addr`, `request.method`, `request.path`) and the error reason to ensure all anomalous input attempts are documented for security review.
## 2026-04-27 - [Silent Discarding of HTTP Exceptions]
**Vulnerability:** The global API error handler correctly intercepted `werkzeug.exceptions.HTTPException` instances (like `404 Not Found`, `405 Method Not Allowed`, `413 Payload Too Large`) to return standardized JSON responses, but it failed to log these events.
**Learning:** Returning appropriate HTTP status codes to the client is insufficient for security monitoring. Without an internal log of these handled HTTP errors, critical security events such as directory brute-forcing, vulnerability scanning (fuzzing), or large payload DoS attempts become invisible to incident responders.
**Prevention:** Even when standard HTTP exceptions are handled and safely returned to the user, always ensure they are explicitly logged internally (e.g., via `app.logger.warning`) alongside the request context (`client_ip`, `method`, `path`) to maintain a comprehensive security audit trail.
