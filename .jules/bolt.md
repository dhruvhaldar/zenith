...
## 2027-04-14 - Scalar vs Array Math Error Handling
**Learning:** When pre-calculating scalar terms to optimize NumPy array operations involving functions like logarithms, using Python's standard `math` module (e.g., `math.log(d)`) is dangerous if the domain might contain invalid values (like negative numbers). While `math.log` raises a breaking `ValueError`, standard NumPy operations (`np.log(d)`) safely propagate `NaN`s and issue a `RuntimeWarning`.
**Action:** When extracting scalar math from an array operation to optimize it, continue using the NumPy math functions (e.g., `np.log()`) instead of the Python `math` module to preserve the expected fallback error behavior, preventing unexpected application crashes on invalid physical data.
