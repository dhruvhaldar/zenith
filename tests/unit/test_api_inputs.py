...
def test_safe_get_float_hpp():
    """Test that multiple values for the same key (HPP) are rejected."""
    with pytest.raises(ValueError, match="Multiple values provided for val, potential HPP attack"):
...
def test_api_rejects_hpp(client):
    """Test that the API endpoints return 400 when multiple values are provided for the same parameter."""
    response = client.get('/api/snr?mag=12&mag=15')
    assert response.status_code == 400
    assert response.get_json()['error'] == "Invalid input parameters"
